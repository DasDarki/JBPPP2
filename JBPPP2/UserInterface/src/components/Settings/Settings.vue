...
import type {PrioLaunchers} from "@/libs/settings-manager";
import {getLocation, getPrioLauncher, setLocation, setPrioLauncher} from "@/libs/settings-manager";
import type {LocationList} from "@/@types/models";

const props = defineProps<{
  location: LocationList;
}>();
const emit = defineEmits<{
  (event: "change-loc", loc: string): void;
}>();
...
const currentLoc = ref(getLocation());

const locations = computed<{name: string, val: string}[]>(() => {
  const locs: {name: string, val: string}[] = [];

  for (const name in props.location) {
    if (props.location.hasOwnProperty(name)) {
      locs.push({name, val: props.location[name]["country-file"]});
    }
  }
...
function show() {
  ...
  currentPrioLauncher.value = getPrioLauncher();
...
function onPrioLauncherChange() {
  setPrioLauncher(currentPrioLauncher.value);
}

function onLocChange() {
  setLocation(currentLoc.value);

  emit("change-loc", currentLoc.value);
}
    ...
    <div class="settings-card card shadowed" :class="{opening: visible}">
      <div class="container" :class="{opened: opened}">
        ...
        <div class="control">
          ...
          <select class="input" v-model="currentLoc" @change="onLocChange">
            <option v-for="l in locations" :value="l.val">{{l.name}}</option>
          </select>
        </div>
